...
class CreditChinaSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        # creditchina_cust_list=['深圳市红太阳实业有限公司']
        for creditchina_cust in creditchina_cust_list:
            default_data = {
    ...
    def parse_detail_info(self, response):
        ...
        cust = response.meta['cust']
        results = eval(detail_info.replace('null', 'None').replace('false', 'None'))['data']['results']
        ...
        if results!=None and len(results)>0:
            for result in results:
                if result['数据类别'] == '失信黑名单-法人':
                    item = CreditchinaLoaderItem(item=DishonestyBlacklistItem(), response=response)
                    item.add_value('batch_date', batch_date)
                    item.add_value('cust_name', cust)
                    item.add_value('data_source', result['数据来源'])
                    item.add_value('case_no', result['案号'])
                    item.add_value('dishonesty_cust_name', result['失信被执行人名称'])
                    item.add_value('legal_person', result['企业法人姓名'])
                    item.add_value('exec_court', result['执行法院'])
                    item.add_value('area_name', result['地域名称'])
                    item.add_value('exec_gist_no', result['执行依据文号'])
                    item.add_value('exec_gist_org', result['作出执行依据单位'])
                    item.add_value('writ_content', result['法律生效文书确定的义务'])
                    item.add_value('performance_status', result['被执行人的履行情况'])
                    item.add_value('dishonesty_cust_specific_status', result['失信被执行人具体情形'])
                    item.add_value('issue_date', result['发布时间'])
                    item.add_value('register_date', result['立案时间'])
                    item.add_value('performanced_part', result['已履行部分'])
                    item.add_value('unperformanced_part', result['未履行部分'])
                    item.add_value('data_update_date', result['最新更新日期'])
                    item.add_value('table_name', 'dishonesty_blacklist')
                    yield item.load_item()
                else:
                    item = CreditchinaLoaderItem(item=DishonestyBlacklistItem(), response=response)
                    item.add_value('batch_date', batch_date)
                    item.add_value('cust_name', cust)
                    item.add_value('data_source', '')
                    item.add_value('case_no', '')
                    item.add_value('dishonesty_cust_name', '')
                    item.add_value('legal_person', '')
                    item.add_value('exec_court', '')
                    item.add_value('area_name', '')
                    item.add_value('exec_gist_no', '')
                    item.add_value('exec_gist_org', '')
                    item.add_value('writ_content', '')
                    item.add_value('performance_status', '')
                    item.add_value('dishonesty_cust_specific_status', '')
                    item.add_value('issue_date', '')
                    item.add_value('register_date', '')
                    item.add_value('performanced_part', '')
                    item.add_value('unperformanced_part', '')
                    item.add_value('data_update_date', '')
                    item.add_value('table_name', 'dishonesty_blacklist')
                    yield item.load_item()
                if result['数据类别'] == '重大税收违法案件当事人名单':
                    item = CreditchinaLoaderItem(item=SeriousRevenueLawlessCustListItem(), response=response)
                    item.add_value('batch_date', batch_date)
                    item.add_value('cust_name', cust)
                    item.add_value('data_source', result['数据来源'])
                    item.add_value('taxer_name', result['纳税人名称'])
                    item.add_value('taxer_id', result['纳税人识别码'])
                    item.add_value('org_code', result['组织机构代码'])
                    item.add_value('register_addr', result['注册地址'])
                    item.add_value('legal_person_name', result['法定代表人或者负责人姓名'])
                    item.add_value('legal_person_sex', result['法定代表人或者负责人性别'])
                    item.add_value('legal_person_cert', result['法定代表人或者负责人证件名称'])
                    item.add_value('financing_person_name', result['负有直接责任的财务负责人姓名'])
                    item.add_value('financing_person_sex', result['负有直接责任的财务负责人性别'])
                    item.add_value('financing_person_cert', result['负有直接责任的财务负责人证件名称'])
                    item.add_value('intermediary_info', result['负有直接责任的中介机构信息及其从业人员信息'])
                    item.add_value('case_nature', result['案件性质'])
                    item.add_value('lawless_fact', result['主要违法事实'])
                    item.add_value('punish_status', result['相关法律依据及税务处理处罚情况'])
                    item.add_value('case_report_date', result['案件上报期'])
                    item.add_value('data_update_date', result['最新更新日期'])
                    item.add_value('table_name', 'serious_revenue_lawless_cust_list')
                    yield item.load_item()
                else:
                    item = CreditchinaLoaderItem(item=SeriousRevenueLawlessCustListItem(), response=response)
                    item.add_value('batch_date', batch_date)
                    item.add_value('cust_name', cust)
                    item.add_value('data_source', '')
                    item.add_value('taxer_name', '')
                    item.add_value('taxer_id', '')
                    item.add_value('org_code', '')
                    item.add_value('register_addr', '')
                    item.add_value('legal_person_name', '')
                    item.add_value('legal_person_sex', '')
                    item.add_value('legal_person_cert', '')
                    item.add_value('financing_person_name', '')
                    item.add_value('financing_person_sex', '')
                    item.add_value('financing_person_cert', '')
                    item.add_value('intermediary_info', '')
                    item.add_value('case_nature', '')
                    item.add_value('lawless_fact', '')
                    item.add_value('punish_status', '')
                    item.add_value('case_report_date', '')
                    item.add_value('data_update_date', '')
                    item.add_value('table_name', 'serious_revenue_lawless_cust_list')
                    yield item.load_item()
                if result['数据类别'] == '财政部采购不良记录数据':
                    item = CreditchinaLoaderItem(item=PurchasingBadnessRecordItem(), response=response)
                    item.add_value('batch_date', batch_date)
                    item.add_value('cust_name', cust)
                    item.add_value('data_source', result['数据来源'])
                    item.add_value('supplier_name', result['供应商或代理机构名称'])
                    item.add_value('supplier_addr', result['地址'])
                    item.add_value('lawless_status', result['不良行为的具体情形'])
                    item.add_value('punish_result', result['处罚结果'])
                    item.add_value('punish_gist', result['处罚依据'])
                    item.add_value('punish_date', result['处罚（记录）日期'])
                    item.add_value('exec_org', result['执法（记录）单位'])
                    item.add_value('punish_end_date', result['处罚结束时间'])
                    item.add_value('data_update_date', result['最新更新日期'])
                    item.add_value('table_name', 'purchasing_badness_record')
                    yield item.load_item()
                else:
                    item = CreditchinaLoaderItem(item=PurchasingBadnessRecordItem(), response=response)
                    item.add_value('batch_date', batch_date)
                    item.add_value('cust_name', cust)
                    item.add_value('data_source', '')
                    item.add_value('supplier_name', '')
                    item.add_value('supplier_addr', '')
                    item.add_value('lawless_status', '')
                    item.add_value('punish_result', '')
                    item.add_value('punish_gist', '')
                    item.add_value('punish_date', '')
                    item.add_value('exec_org', '')
                    item.add_value('punish_end_date', '')
                    item.add_value('data_update_date', '')
                    item.add_value('table_name', 'purchasing_badness_record')
                    yield item.load_item()
        else:
